...
create table if not exists users_user_roles (user_id integer not null, user_roles_id integer not null);

create sequence user_roles_seq start with 1 increment by 50;

alter table if exists users_user_roles
...
insert into users_user_roles(user_id, user_roles_id) values (1, 1);